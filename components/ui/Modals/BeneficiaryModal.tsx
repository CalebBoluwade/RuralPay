...
const BeneficiaryModal: React.FC<Props> = ({ visible, onClose, onSelect }) => {
  ...
  const handleSelect = async (item: Beneficiary) => {
    ...
    const idx = list.findIndex((b) => b.accountNumber === item.accountNumber && b.bankCode === item.bankCode);
    if (idx >= 0) {
      list[idx].useCount += 1;
    ...
    const sorted = list.sort((a, b) => b.useCount - a.useCount).slice(0, FREQ_LIMIT);
  ...
  const inputBg = isDark ? "bg-white/10 border-white/20 text-white" : "bg-gray-100 border-gray-200 text-gray-900";
      ...
      <SafeAreaView style={{ flex: 1 }} className={bg}>
        <View className="flex-1">
          ...
          <View className={`p-5 border-b ${borderColor}`}>
            <Text className={`text-xl font-bold text-center mb-4 ${textPrimary}`}>
              Select Beneficiary
            </Text>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  ...
                  >
                    <Text className={`font-bold text-sm ${isDark ? "text-emerald-400" : "text-emerald-700"}`}>
                      {item.accountName.charAt(0).toUpperCase()}
                    </Text>
                  </View>
                  <View className="flex-1">
                    <Text className={`font-medium ${textPrimary}`}>{item.accountName}</Text>
                    <Text className={`text-xs mt-0.5 ${textSecondary}`}>
                      {item.accountNumber} · {item.bankName}
                    </Text>
                  </View>
                  {showFrequent && item.useCount > 0 && (
                    <Text className={`text-xs ${textSecondary}`}>{item.useCount}×</Text>
                  )}
                </Pressable>
              )}
              ListEmptyComponent={
                <View className="flex-1 items-center justify-center py-16">
                  <Text className={`text-base ${textSecondary}`}>
                    {showFrequent ? "No frequent beneficiaries yet" : "No beneficiaries found"}
                  </Text>
                </View>
              }
            />
          ...
          <Pressable
            ...
            onPress={onClose}
          >
            <Text className={`text-center font-semibold ${isDark ? "text-white" : "text-gray-600"}`}>
              Cancel
            </Text>
          </Pressable>
        </View>
