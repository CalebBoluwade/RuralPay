...
const SpendingTracker = () => {
  ...
  const loadSpendingData = async () => {
    try {
      ...
      const debits = data.transactions.filter((tx) =>
        tx.txType?.includes("DEBIT"),
      );
    ...
    >
      ...
      <ScrollView className="flex-1 px-5" showsVerticalScrollIndicator={false}>
        ...
        <View className={`rounded-2xl p-5 mb-6 mt-4 ${cardClass}`}>
          ...
          <Text
            className={`text-xs ${isDark ? "text-lime-400" : "text-lime-600"}`}
          >
            {spendingLabel}
          </Text>
        </View>
              ...
              {categories.map(([category, amount], index) => {
                ...
                return (
                  ...
                  >
                    <View className="flex-row items-center gap-3 mb-3">
                      <View
                        className={`w-12 h-12 rounded-xl items-center justify-center ${color.bg}`}
                      >
